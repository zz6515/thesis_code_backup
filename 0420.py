...
class mainGUI(QMainWindow, GUI0):
    ...
    def item_clicked(self, event):
        if event.button() == Qt.RightButton:  # 判断鼠标右键点击
            ...
            if isinstance(item, QGraphicsRectItem):  # 判断点击对象是否为图元的实例
                self.showpanel.append("已删除")
    ...
    def ndvi(self):
       ...
       图像预处理：
        ...
        out_dataset = tif_driver.Create(prepicture_path, dataset.RasterXSize, dataset.RasterYSize, 1,
    ...
    def gdalClip(self):
        ...
        for x, y in self.points:
            ...
            tif_driver = gdal.GetDriverByName("GTiff")
            out_dataset = tif_driver.Create('Image_Clipped/sample' + '-' + str(offset_x) + '-' + str(offset_y) + '.tif',
    ...
    def exec_clipWithShp(self):
        ...
        for index, feat in enumerate(layer):
            ...
            tif_driver = gdal.GetDriverByName("GTiff")
            out_dataset = tif_driver.Create('clipWithShp/sample-' + str(index) + '.tif', block_sizeX, block_sizeY,
    ...
    def createShpAndClip(self):
        ...
        tif_driver = gdal.GetDriverByName("GTiff")
        out_dataset = tif_driver.Create('create_shp_clip/sample' + '-' + str(xmin) + '-' + str(ymin) + '.tif',
                                        self.block_size, self.block_size, 3, band1.DataType)
        # 将计算后的值组装为一个元组，以方便设置
        dst_transform = (
